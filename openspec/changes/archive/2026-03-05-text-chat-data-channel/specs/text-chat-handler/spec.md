...
The agent SHALL send received text messages directly to the LLM via `AgentSession.generate_reply(user_input=text)`, bypassing the STT pipeline entirely. The text message SHALL appear in the same conversation context as voice messages.

#### Scenario: Text routed to LLM
- **WHEN** a valid `chat_message` is received
- **THEN** the agent calls `generate_reply` with the user's text, and the LLM generates a response using the full conversation history (including prior voice messages)
...
The agent SHALL NOT invoke TTS for responses generated from text input. When a response is triggered by a `chat_message`, the `TwypeAgent.tts_node` SHALL return `None` to skip speech synthesis.
